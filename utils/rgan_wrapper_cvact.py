...
class RGANWrapper(BaseModel):
    ...
    def backward_R(self, epoch):
        ...
        if epoch <= 20:
            self.r_loss = self.compute_cvact_loss(self.fake_street_out, self.street_out, self.in_batch_dis, self.posDistSqThr, loss_weight=self.opt.lambda_sm, hard_topk_ratio=self.opt.hard_topk_ratio)
        elif epoch > 20 and epoch <=40:
            self.r_loss = self.compute_cvact_loss(self.fake_street_out, self.street_out, self.in_batch_dis, self.posDistSqThr, loss_weight=self.opt.lambda_sm, hard_topk_ratio=self.opt.hard_decay1_topk_ratio)
        elif epoch > 40 and epoch <=60:
            self.r_loss = self.compute_cvact_loss(self.fake_street_out, self.street_out, self.in_batch_dis, self.posDistSqThr, loss_weight=self.opt.lambda_sm, hard_topk_ratio=self.opt.hard_decay2_topk_ratio)
        elif epoch > 60:
            self.r_loss = self.compute_cvact_loss(self.fake_street_out, self.street_out, self.in_batch_dis, self.posDistSqThr, loss_weight=self.opt.lambda_sm, hard_topk_ratio=self.opt.hard_decay3_topk_ratio)
    ...
    def backward_G(self, epoch):
        ...
        if epoch <= 20:
            self.ret_loss = self.compute_cvact_loss(self.fake_street_out, self.street_out, self.in_batch_dis, self.posDistSqThr, loss_weight=self.opt.lambda_sm,
                                                        hard_topk_ratio=self.opt.hard_topk_ratio) * self.opt.lambda_ret1
        elif epoch > 20 and epoch <=40:
            self.ret_loss = self.compute_cvact_loss(self.fake_street_out, self.street_out, self.in_batch_dis, self.posDistSqThr,
